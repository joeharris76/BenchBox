...
class TestQueryHeatmap:
    def test_builds_basic_chart(self):
        matrix = [[10.0, 12.0], [8.5, 9.2]]
        fig = QueryHeatmap(matrix=matrix, queries=["Q1", "Q2"], platforms=["duckdb", "snowflake"]).figure()
    ...
    def test_single_row_matrix(self):
        matrix = [[5.0, 6.0, 7.0]]
        fig = QueryHeatmap(matrix=matrix, queries=["Q1"], platforms=["a", "b", "c"]).figure()
        assert fig.data[0].type == "heatmap"

    def test_default_labels(self):
        """Verify default title and colorbar accurately describe execution time data."""
        matrix = [[100.0, 200.0]]
        chart = QueryHeatmap(matrix=matrix, queries=["Q1"], platforms=["a", "b"])
        assert chart.title == "Query Execution Time Heatmap"
        assert chart.colorbar_title == "Execution Time (ms)"
