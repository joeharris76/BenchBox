...
class TestGetDefaultDataDirectory:
    ...
    @patch.dict(os.environ, {"BENCHBOX_DATA_DIR": "/env/path"})
    def test_get_default_data_directory_returns_path_object(self):
...
class TestPathUtilsIntegration:
    ...
    def test_datagen_and_results_paths_independence(self):
        ...
        assert str(datagen_path).startswith(base_dir)
        assert str(results_path).startswith(base_dir)

        # Verify expected structures
        assert datagen_path == Path(f"{base_dir}/tpch_sf1")
        assert results_path == Path(f"{base_dir}/results/tpch_20250115_120000")
