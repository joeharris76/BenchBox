...
class TestTPCDIFullBenchmarkIntegration:
    ...
    def test_performance_regression_baseline(self, small_scale_config, test_database):
        ...
        baseline_file = Path(__file__).parent / "tpcdi_performance_baseline.json"
        try:
    ...
    def test_memory_usage_and_resource_consumption(self, small_scale_config, test_database):
        ...
        # Check memory usage is reasonable (allow up to 500MB for small scale)
        ...
        memory_growth = memory_measurements["after_etl"] - baseline_memory
        assert memory_growth < 200, f"Potential memory leak detected: {memory_growth:.1f}MB growth"
...
class TestTPCDIPerformanceAndScalability:
    ...
    def test_memory_usage_resource_consumption_patterns(self, temp_dir, test_database):
        ...
        memory_growth = resource_history[-1]["memory_mb"] - resource_history[0]["memory_mb"]

        # Resource usage should be reasonable for scale factor 0.1
        assert peak_memory < 1000, f"Peak memory usage too high: {peak_memory:.1f}MB"
