...
3. Prepare curated public tree
4. Build packages with SOURCE_DATE_EPOCH
5. Verify artifacts and run smoke tests
6. Finalize with git commit/tag (both with normalized timestamps)
7. Archive artifacts
...
def run_pre_flight_checks(source: Path, version: str) -> bool:
    ...
    try:
        ...
        if source_version != version:
            ...
            confirm_or_exit("Continue with mismatched versions?")
        else:
            print(f"✓ Version matches source: {version}")
    except Exception as e:
        print(f"⚠️  Warning: Could not verify source version: {e}")
        confirm_or_exit("Continue without version verification?")
...
def main() -> int:
    ...
    confirm_or_exit(f"Proceed with release v{version}?")

    # Step 1: Pre-flight checks
    if not args.skip_preflight:
        if not run_pre_flight_checks(source, version):
            print("\n❌ Pre-flight checks failed")
            return 1
        confirm_or_exit("Pre-flight checks passed. Continue?")

    # Step 2: Prepare public tree
    run_step(
        "prepare_release.py",
        [
            str(target),
        ...
        "Step 1/5: Preparing Public Tree",
    )

    # Step 2: Size safety check
    ...
    run_step(
        "verify_release.py",
        verify_args,
        "Step 4/5: Verifying Artifacts & Running Smoke Tests",
    )

    # Step 5: Finalize (git commit/tag)
    finalize_args = [
        str(target),
        "--version",
        version,
